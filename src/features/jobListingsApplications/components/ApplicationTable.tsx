...
const DisabledToolbar = <T,>({ table }: { table: Table<T> }) => {
  return <Toolbar disabled table={table} />;
};
...
export const SkeletonApplicationTable = () => {
  return (
    <ApplicationTable
      applications={[]}
...
const getColumns = (
  ...
  return [
    ...
    },
    {
      ...
      filterFn: ({ original }, _, value) => value.includes(original.rating),
      cell: ({ row }) => (
        <RatingCell
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className={cn("cursor-pointer -ml-3", isPending && "opacity-50")}
        >
...
const ActionCell = ({
  ...
  coverLetterMarkdown: React.ReactNode | null;
  resumeMarkdown: React.ReactNode | null;
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          {coverLetterMarkdown ? (
            <DropdownMenuItem
              onClick={() => setOpenModal("coverLetter")}
              className="cursor-pointer"
            >
              View Cover Letter
            </DropdownMenuItem>
          ...
          <DialogContent className="lg:max-w-5xl md:max-w-3xl max-h-[calc(100%-2rem)] overflow-hidden flex flex-col">
            <DialogHeader>
              <div className="flex gap-4 items-center">
                ...
                {resumeUrl && (
                  <Button asChild className="cursor-pointer self-start">
                    <Link
                      href={resumeUrl}
                      target="_blank"
...
const StageDetails = ({ stage }: { stage: ApplicationStage }) => {
  return (
