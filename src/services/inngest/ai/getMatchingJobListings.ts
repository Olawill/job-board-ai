...
export const getMatchingJobListings = async (
  ...
  const NO_JOBS = "No jobs matching your query found.";

  const agent = createAgent({
    name: "Job Matching Agent",
    description: "Agent for matching users with job listings",
    system: `You are en expert at matching people with jobs based on their specific experience, and requirements. The provided user prompt will be a description that can include information about themselves as well as what they are looking for in a job. ${
      maxNumberOfJobs
